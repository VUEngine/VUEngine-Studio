...
import { isWindows, MessageService } from "@theia/core/lib/common";
...
export async function flashCommand(
    ...
    if (!connectedFlashCart) {
        ...
        flash(messageService, preferenceService, terminalService, workspaceService, connectedFlashCart);
    }
}

async function detectFlashCart(flashCartConfigs: FlashCartConfig[]): Promise<ConnectedFlashCart | undefined> {
...
async function flash(
    ...
    const romPath =
        connectedFlashCart.config.padRom && padRom(workspaceService, connectedFlashCart.config.size)
...
function getRomPath(workspaceService: WorkspaceService): string {
