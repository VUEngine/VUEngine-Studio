import * as React from "react";
import { basename } from "path";
import { inject, injectable, postConstruct } from "inversify";
import { ReactWidget } from "@theia/core/lib/browser/widgets/react-widget";
import { VesStateModel } from "../../common/vesStateModel";
import { CommandService } from "@theia/core";
import { VesFlashCartsCommand } from "../commands";
import { VesProcessService } from "../../../common/process-service-protocol";
import { VesFlashCartsPreference } from "../preferences";

@injectable()
export class VesFlashCartWidget extends ReactWidget {
  @inject(CommandService) private readonly commandService: CommandService;
  @inject(VesProcessService) private readonly vesProcessService: VesProcessService;
  @inject(VesStateModel) private readonly vesState: VesStateModel;

  static readonly ID = "vesFlashCartWidget";
  static readonly LABEL = "Flash Cart";

  @postConstruct()
  protected async init(): Promise<void> {
    this.id = VesFlashCartWidget.ID;
    this.title.label = VesFlashCartWidget.LABEL;
    this.title.caption = VesFlashCartWidget.LABEL;
    this.title.iconClass = "fa fa-usb";
    this.title.closable = true;
    this.update();

    this.vesState.onDidChangeConnectedFlashCart(() => this.update());
    this.vesState.onDidChangeIsFlashing(() => this.update());
    this.vesState.onDidChangeIsFlashQueued(() => this.update());
    this.vesState.onDidChangeFlashingProgress(() => this.update());
  }

  protected render(): React.ReactNode {
    return <>
      {this.vesState.connectedFlashCart
        ? <>
          <div className="flashCartTitle">{this.vesState.connectedFlashCart.config.name}</div>
          <div className="flashCartInfo">
            <div>
              <i className="fa fa-fw fa-microchip"></i> {this.vesState.connectedFlashCart.config.size} MBit
              ({this.vesState.connectedFlashCart.config.padRom ? "Auto-Padding" : "No Auto-Padding"})
            </div>
            <div>
              <i className="fa fa-fw fa-usb"></i> {this.vesState.connectedFlashCart.config.vid}:{this.vesState.connectedFlashCart.config.pid}<br />
              {this.vesState.connectedFlashCart.config.manufacturer}<br />
              {this.vesState.connectedFlashCart.config.product}
            </div>
            <div>
              <i className="fa fa-fw fa-terminal"></i> {basename(this.vesState.connectedFlashCart.config.path)} {this.vesState.connectedFlashCart.config.args}
            </div>
          </div>
          {this.vesState.connectedFlashCart.config.image &&
            <div className="cartImgWrapper">
              <img
                src={this.vesState.connectedFlashCart.config.image}
                style={this.vesState.connectedFlashCart.config.name === VesFlashCartsPreference.property.default[1].name ? {
                  backgroundImage: "url(data:image/png;base64,)",
                  backgroundPosition: "69% 28%",
                  backgroundRepeat: "no-repeat",
                  backgroundSize: "76%",
                } : {}} />
            </div>
          }
          <div className="flashingPanel">
            {this.vesState.isFlashQueued &&
              <>
                <div>
                  <i className="fa fa-fw fa-hourglass-half"></i> <em>Flashing is queued and will start once the build is ready</em>
                </div>
              </>
            }
            {this.vesState.isFlashing > 0 &&
              <>
                <div className="progressBar">
                  <div style={{ width: this.vesState.flashingProgress.progress + "%" }}>
                    <span>
                      {this.vesState.flashingProgress.progress > -1 &&
                        this.vesState.flashingProgress.progress + "%"
                      }
                    </span>
                  </div>
                </div>
                <div>
                  <i className="fa fa-fw fa-refresh fa-spin"></i> {this.vesState.flashingProgress.step}...
                </div>
                <button className="theia-button secondary" onClick={() => this.vesProcessService.killProcess(this.vesState.isFlashing)}>
                  Cancel
                </button>
              </>
            }
            {!this.vesState.isFlashQueued && !this.vesState.isFlashing
              ? <>
                <button className="theia-button flash" onClick={() => this.commandService.executeCommand(VesFlashCartsCommand.id)}>
                  <i className="fa fa-usb"></i> Flash
                </button>
              </>
              : <></>
            }
          </div>
        </>
        : <>
          <div><em>No flash cart connected.</em></div>
        </>
      }
    </>;
  }

}
