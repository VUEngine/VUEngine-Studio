...
import {
  ...
  DefaultSecondaryGridCellCount,
  DefaultSecondaryGridStrokeColor,
  DefaultSecondaryGridStrokeWidth,
...
export default class GridLayer extends BaseLayer {
  ...
  private isGridVisible: boolean | undefined;
  ...
  private isSecondaryGridVisible: boolean | undefined;
  private secondaryGridCellCount: number = DefaultSecondaryGridCellCount;
  private secondaryGridStrokeColor: string = DefaultSecondaryGridStrokeColor;
  private secondaryGridStrokeWidth: number = DefaultSecondaryGridStrokeWidth;
  ...
  getIsGridVisible(): boolean | undefined {
    return this.isGridVisible;
  }

  setIsSecondaryGridVisible(isSecondaryGridVisible: boolean): void {
    if (isSecondaryGridVisible !== undefined) {
      this.isSecondaryGridVisible = isSecondaryGridVisible;
    }
  }

  getIsSecondaryGridVisible(): boolean | undefined {
    return this.isSecondaryGridVisible;
  }

  setSecondaryGridCellCount(secondaryGridCellCount: number): void {
    if (secondaryGridCellCount !== undefined && secondaryGridCellCount >= 1) {
      this.secondaryGridCellCount = Math.floor(secondaryGridCellCount);
    }
  }

  getSecondaryGridCellCount(): number {
    return this.secondaryGridCellCount;
  }

  setSecondaryGridStrokeColor(secondaryGridStrokeColor: string): void {
    if (secondaryGridStrokeColor !== undefined) {
      this.secondaryGridStrokeColor = secondaryGridStrokeColor;
    }
  }

  setSecondaryGridStrokeWidth(secondaryGridStrokeWidth: number): void {
    if (secondaryGridStrokeWidth !== undefined) {
      this.secondaryGridStrokeWidth = secondaryGridStrokeWidth;
    }
  }
  ...
  getHoveredButton(): ButtonDirection | null {
  ...
  ): { worldPos: Coord; screenPos: Coord; } {
  ...
  renderSelection(area: { startWorldPos: Coord; endWorldPos: Coord }): void {
    const ctx = this.ctx;
   ...
   * 2. isGridVisible or isSecondaryGridVisible is changed
   * 3. gridStrokeColor or secondaryGridStrokeColor is changed
   * 4. gridStrokeWidth or secondaryGridStrokeWidth is changed
   * 5. secondaryGridCellCount is changed
   * 6. panZoom is changed
   * @returns {void}
   */
  render(): void {
    ...
    if (!this.isGridVisible && !this.isSecondaryGridVisible) {
      return;
    }
    ctx.save();
    if (this.isGridVisible) {
      this.drawGridLines(correctedScreenPoint, squareLength);
    }
    if (this.isSecondaryGridVisible) {
      this.drawSecondaryGridLines(correctedScreenPoint, squareLength);
    }
    ctx.restore();
  }

  private drawGridLines(
    correctedScreenPoint: Coord,
    squareLength: number,
  ): void {
    const ctx = this.ctx;
  ...
  private drawSecondaryGridLines(
    correctedScreenPoint: Coord,
    squareLength: number,
  ): void {
    const ctx = this.ctx;
    const cellCount = this.secondaryGridCellCount;
    const halfStroke = this.secondaryGridStrokeWidth / 2;
    ctx.lineWidth = this.secondaryGridStrokeWidth;
    ctx.strokeStyle = this.secondaryGridStrokeColor;

    const columns = [];
    for (let i = 0; i <= this.columnCount; i += cellCount) {
      columns.push(i);
    }
    if (columns[columns.length - 1] !== this.columnCount) {
      columns.push(this.columnCount);
    }
    for (const i of columns) {
      ctx.beginPath();
      ctx.moveTo(
        correctedScreenPoint.x + i * squareLength,
        correctedScreenPoint.y - halfStroke,
      );
      ctx.lineTo(
        correctedScreenPoint.x + i * squareLength,
        correctedScreenPoint.y + this.rowCount * squareLength + halfStroke,
      );
      ctx.stroke();
      ctx.closePath();
    }
    ...
    const rows = [];
    for (let j = 0; j <= this.rowCount; j += cellCount) {
      rows.push(j);
    }
    if (rows[rows.length - 1] !== this.rowCount) {
      rows.push(this.rowCount);
    }
    for (const j of rows) {
      ctx.beginPath();
      ctx.moveTo(
        correctedScreenPoint.x - halfStroke,
        correctedScreenPoint.y + j * squareLength,
      );
      ctx.lineTo(
        correctedScreenPoint.x + this.columnCount * squareLength + halfStroke,
        correctedScreenPoint.y + j * squareLength,
      );
      ctx.stroke();
      ctx.closePath();
