...
export default class DataLayer extends BaseLayer {
  ...
  getDimensions(): { columnCount: number; rowCount: number; } {
    return {
  ...
  getLayer(layerId: string): DottingDataLayer | null {
    const layer = this.layers.find(l => l.getId() === layerId);
    if (layer) {
      return layer;
    } else {
      return null;
  ...
  isolateLayer(layerId: string): void {
    ...
    this.layers.forEach(otherLayer => otherLayer.setIsVisible(false));
    layer.setIsVisible(true);
  }
  ...
  getCapturedImageBitmap(): ImageBitmap | null {
  ...
  colorPixels(data: Array<PixelModifyItem>, layerId?: string): { dataForAction: ColorChangeItem[]; totalAddedColumnIndices: number[]; totalAddedRowIndices: number[]; } {
    if (!layerId) {
      if (this.layers.length > 1) {
        throw new Error('Must specify layerId when there are multiple layers');
      }
  ...
  addRow(rowIndex: number): { validRowIndex: null; } {
  ...
  addColumn(columnIndex: number): { validColumnIndex: null; } {
  ...
  deleteRow(rowIndex: number): { swipedPixels: PixelModifyItem[]; validRowIndex: null; } {
    let validRowIndex = null;
  ...
  deleteColumn(columnIndex: number): { swipedPixels: PixelModifyItem[]; validColumnIndex: null; } {
    let validColumnIndex = null;
    const firstRowKey = this.getData().keys().next().value;
    const firstRow =
      firstRowKey !== undefined ? this.getData().get(firstRowKey) : undefined;
    // This is to prevent deleting a non-existent column
    if (!firstRow?.has(columnIndex)) {
      return {
        swipedPixels: [],
        validColumnIndex,
      };
  ...
  }): { validRowIndices: number[]; validColumnIndices: number[]; } {
  ...
  clear(): { layerId: string; data: Array<PixelModifyItem>; }[] {
  ...
  }): { swipedPixels: PixelModifyItem[]; validColumnIndices: number[]; validRowIndices: number[]; } {
    const validRowIndices: Array<number> = [];
    const validColumnIndices: Array<number> = [];
  ...
  render(): { rowKeys: number[]; columnKeys: number[]; } {
    const ctx = this.ctx;
