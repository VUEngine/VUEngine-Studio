...
export default class Editor extends EventDispatcher {
  ...
  private originalLayerIdsInOrderForHistory: Array<string> = [];
  ...
  setIsGridFixed(isGridFixed: boolean | undefined): void {
  ...
  setGridStrokeColor(color: string | undefined): void {
    if (color !== undefined) {
  ...
  setGridStrokeWidth(width: number | undefined): void {
    if (width !== undefined) {
  ...
  setDefaultPixelColor(color: string | undefined): void {
    if (color === undefined) {
      return;
    }
  ...
  setGridSquareLength(length: number | undefined): void {
  ...
  setMinScale(minScale: number | undefined): void {
  ...
  setMaxScale(maxScale: number | undefined): void {
  ...
  setIsSecondaryGridVisible(
    isSecondaryGridVisible: boolean | undefined,
  ): void {
    if (isSecondaryGridVisible !== undefined) {
      this.gridLayer.setIsSecondaryGridVisible(isSecondaryGridVisible);
    }
    this.renderGridLayer();
  }

  setSecondaryGridCellCount(secondaryGridCellCount: number | undefined): void {
    if (secondaryGridCellCount !== undefined) {
      this.gridLayer.setSecondaryGridCellCount(secondaryGridCellCount);
    }
    this.renderGridLayer();
  }

  setSecondaryGridStrokeColor(color: string | undefined): void {
    if (color !== undefined) {
      this.gridLayer.setSecondaryGridStrokeColor(color);
    }
    this.renderGridLayer();
  }

  setSecondaryGridStrokeWidth(width: number | undefined): void {
    if (width !== undefined) {
      this.gridLayer.setSecondaryGridStrokeWidth(width);
    }
    this.renderGridLayer();
  }

  setIsGridVisible(isGridVisible: boolean | undefined): void {
  ...
  setIsPanZoomable(isPanZoomable: boolean | undefined): void {
    if (isPanZoomable !== undefined) {
      this.isPanZoomable = isPanZoomable;
    }
  }

  setIsDrawingEnabled(isDrawingEnabled: boolean | undefined): void {
    if (isDrawingEnabled !== undefined) {
      this.isDrawingEnabled = isDrawingEnabled;
    }
  }

  setIsInteractionApplicable(isInteractionApplicable: boolean | undefined): void {
  ...
  styleMouseCursor = (mouseCoord: Coord) => {
    ...
    let hoveredButton: ButtonDirection | null =
      this.detectButtonClicked(mouseCoord);
  ...
  getLayers(): { id: string; data: DottingData; }[] {
  ...
  getLayersAsArray(): { id: string; data: PixelModifyItem[][]; }[] {
    return this.dataLayer.getLayers().map(layer => ({
        id: layer.getId(),
  ...
  removeLayer(layerId: string, shouldRecordAction = false): void {
  ...
  setPanZoom({
    ...
    baseColumnCount,
    baseRowCount,
  }: Partial<PanZoom> & { baseColumnCount?: number; baseRowCount?: number }): void {
    const columnCount = baseColumnCount
      ? baseColumnCount
      : this.dataLayer.getColumnCount();
    const rowCount = baseRowCount ? baseRowCount : this.dataLayer.getRowCount();
  ...
  private relayDataDimensionsToLayers(): void {
  ...
  // 1. colorChangeAction
  // 2. sizeChangeAction
  private relayInteractionDataToDataLayer(): boolean {
    const interactionLayer = this.interactionLayer;
  ...
  onMouseMove(evt: TouchyEvent): void {
    ...
    if (
      ...
      const previewCoord = getMouseCartCoord(
        evt,
        this.element,
        this.panZoom,
        this.dpr,
    ...
    if (this.mouseMode === MouseMode.NULL) {
      ...
      const drawingRowIndices = Array.from(
        this.dataLayer.getRowKeyOrderMap().keys(),
      );
      // columnKeyOrderMap is a sorted map of columnKeys
      const drawingColumnIndices = Array.from(
        this.dataLayer.getColumnKeyOrderMap().keys(),
      );
      const drawingPixelIndex = getPixelIndexFromMouseCartCoord(
        mouseCartCoord,
        drawingRowIndices,
        drawingColumnIndices,
        this.gridSquareLength,
      );
      ...
      if (this.brushTool === BrushTool.SELECT) {
        ...
        if (drawingPixelIndex) {
          this.drawPixelInInteractionLayer(
            drawingPixelIndex.rowIndex,
            drawingPixelIndex.columnIndex,
            this.interactionLayer.getBrushPattern(),
          );
      ...
      for (const i of allRowKeys) {
        ...
        if (options && options.isGridVisible) {
          for (let column = 0; column <= this.getColumnCount(); column++) {
            ...
            imageContext.moveTo(column * this.gridSquareLength, 0);
            imageContext.lineTo(
              column * this.gridSquareLength,
  ...
  renderAll(): void {
    ...
    clearTimeout(this.renderDataLayerTimeout ?? undefined);
