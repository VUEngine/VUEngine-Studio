 ...
 * @param data
 */
export const getRowKeysFromData = (data: DottingData): Array<number> => Array.from(data.keys()).sort((a, b) => a - b);
...
export const getRowCountFromData = (data: DottingData): number => data.size;
...
export const validateLayers = (layers: Array<LayerProps>) => {
  ...
  let measuredColumnCount: number | null = null;
  let measuredRowCount: number | null = null;
  let measuredTopRowIndex: number | null = null;
  let measuredLeftColumnIndex: number | null = null;
