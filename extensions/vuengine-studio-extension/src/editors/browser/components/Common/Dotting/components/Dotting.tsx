...
export interface DottingProps {
  ...
  isSecondaryGridVisible?: boolean;
  secondaryGridCellCount?: number;
  secondaryGridStrokeColor?: string;
  secondaryGridStrokeWidth?: number;
...
export interface DottingRef {
  ...
  getLayers: () => Array<{ id: string; data: DottingData }> | undefined;
  getLayersAsArray: () =>
    | Array<{
      id: string;
      data: Array<Array<PixelModifyItem>>;
...
const Dotting = forwardRef<DottingRef, DottingProps>(function Dotting(
  ...
  useEffect(() => {
    ...
    editor.setIsSecondaryGridVisible(props.isSecondaryGridVisible);
  }, [editor, props.isSecondaryGridVisible]);

  useEffect(() => {
    if (!editor) {
      return;
    }
    editor.setSecondaryGridCellCount(props.secondaryGridCellCount);
  }, [editor, props.secondaryGridCellCount]);

  useEffect(() => {
    if (!editor) {
      return;
    }
    editor.setSecondaryGridStrokeColor(props.secondaryGridStrokeColor);
  }, [editor, props.secondaryGridStrokeColor]);

  useEffect(() => {
    if (!editor) {
      return;
    }
    editor.setSecondaryGridStrokeWidth(props.secondaryGridStrokeWidth);
  }, [editor, props.secondaryGridStrokeWidth]);
  ...
  useEffect(() => {
    ...
    if (dpr) {
      newEditor.scale(dpr, dpr);
    }
    newEditor.setIsGridFixed(props.isGridFixed);
    newEditor.setIsInteractionApplicable(props.isInteractionApplicable);
    newEditor.setIsDrawingEnabled(props.isDrawingEnabled);
    newEditor.setIsPanZoomable(props.isPanZoomable);
    newEditor.setIsGridVisible(props.isGridVisible);
    newEditor.setGridStrokeColor(props.gridStrokeColor);
    newEditor.setGridStrokeWidth(props.gridStrokeWidth);
    newEditor.setSecondaryGridCellCount(props.secondaryGridCellCount);
    newEditor.setIsSecondaryGridVisible(props.isSecondaryGridVisible);
    newEditor.setSecondaryGridStrokeColor(props.secondaryGridStrokeColor);
    newEditor.setSecondaryGridStrokeWidth(props.secondaryGridStrokeWidth);
    newEditor.setGridSquareLength(props.gridSquareLength);
    newEditor.setDefaultPixelColor(props.defaultPixelColor);
    newEditor.setMinScale(props.minScale);
    newEditor.setMaxScale(props.maxScale);
  ...
  const getLayers = useCallback(() => editor?.getLayers(), [editor]);

  const getLayersAsArray = useCallback(() => editor?.getLayersAsArray(), [editor]);
  ...
  const getForegroundCanvas = useCallback(() => foregroundCanvas, [foregroundCanvas]);

  const getBackgroundCanvas = useCallback(() => backgroundCanvas, [backgroundCanvas]);

  const convertWorldPosToCanvasOffset = useCallback(
    (x: number, y: number) => editor?.convertWorldPosToCanvasOffset({
      x,
      y,
    }),
    [editor],
