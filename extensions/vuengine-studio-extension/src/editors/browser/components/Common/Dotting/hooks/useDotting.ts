...
const useDotting = (ref: MutableRefObject<DottingRef | null>) => {
  ...
  const getForegroundCanvas = useCallback(() => ref.current?.getForegroundCanvas(), [ref]);

  const getBackgroundCanvas = useCallback(() => ref.current?.getBackgroundCanvas(), [ref]);

  const convertWorldPosToCanvasOffset = useCallback(
    (x: number, y: number) => ref.current?.convertWorldPosToCanvasOffset(x, y),
    [ref],
  );
