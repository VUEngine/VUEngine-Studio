...
export class DottingDataLayer extends Observable<DottingData> {
  ...
  getDataInfo(): { gridIndices: GridIndices; columnCount: number; rowCount: number; } {
  ...
  deleteRowOfData(rowIndex: number): number {
  ...
  deleteColumnOfData(columnIndex: number): null {
    let validColumnIndex = null;
    this.data.forEach(row => {
      if (!row.has(columnIndex)) {
  ...
  clearData(): { previousPixels: { rowIndex: number; columnIndex: number; color: string; }[]; newPixels: { rowIndex: number; columnIndex: number; color: string; }[]; } {
  ...
  getDataArray(): Array<Array<PixelModifyItem>> {
    ...
    [...this.data.entries()].forEach(([rowIndex, row]) => {
      const rowData: Array<PixelModifyItem> = [];
