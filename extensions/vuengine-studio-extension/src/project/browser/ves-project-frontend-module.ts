...
export default new ContainerModule((bind, unbind, isBound, rebind) => {
    ...
    bind(FrontendApplicationContribution).toService(AssetsBrowserViewContribution);
    bindViewContribution(bind, AssetsBrowserViewContribution);
    bind(AssetsBrowserWidget).toSelf();
    bind(WidgetFactory).toDynamicValue(ctx => ({
        id: AssetsBrowserWidget.ID,
        createWidget: () => ctx.container.get<AssetsBrowserWidget>(AssetsBrowserWidget)
    })).inSingletonScope();
});
